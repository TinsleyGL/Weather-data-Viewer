...
public static VBox buildTable() {
	...
	HBox statsTextBox = new HBox();
	Label statsTextLabel = new Label("as well as statistics from the last year (2017):");
	statsTextBox.setAlignment(Pos.CENTER);
	statsTextBox.getChildren().add(statsTextLabel);
	gridBox.getChildren().add(statsTextBox);
    ...
    Label highMean = new Label("Highest monthly mean maximum temperature (tmax)");
    highMean.setWrapText(true);
    tableGrid.add(highMean, 0, 1);
    Label lowMean = new Label("Lowest monthly mean minimum temperature (tmin)");
    lowMean.setWrapText(true);
    tableGrid.add(lowMean, 0, 2);
    Label totalAir = new Label("Total air frost days");
    totalAir.setWrapText(true);
    tableGrid.add(totalAir, 0, 3);
    Label totalRainfall = new Label("Total rainfall");
    ...
    for(int x = 1, b = 1; x < 13; x++, b++) {		// build data for high mean
        Label lab = new Label(Double.toString(WeatherPOJO.getHighestMeanTempPerMonth
        		(main.weatherPOJOs, b, main.stationNameLabelList[0].getText())) + "\u2103");
        tableGrid.add(lab, x, 1);
        GridPane.setHalignment(lab,HPos.CENTER);
     }
    
    for(int x = 1, b = 1; x < 13; x++, b++) {		// build data for low mean
        Label lab = new Label(Double.toString(WeatherPOJO.getLowestMeanTempPerMonth
        		(main.weatherPOJOs, b, main.stationNameLabelList[0].getText())) + "\u2103");
    ...
    for(int x = 1, b = 1; x < 13; x++, b++) {		//build data for total air frost days
        Label lab = new Label(Integer.toString(WeatherPOJO.getTotalAirFrostDays
        		(main.weatherPOJOs, b, main.stationNameLabelList[0].getText())));
    ...
    for(int x = 1, b = 1; x < 13; x++, b++) {		// build data for total rainfall
        Label lab = new Label(Double.toString(WeatherPOJO.getTotalRainfall
        (main.weatherPOJOs, b, main.stationNameLabelList[0].getText())));
